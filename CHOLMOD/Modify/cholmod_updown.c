...
/* Compute the new LDL' factorization of LDL'+ CC'- DD'.
 * The factor object L need not be an LDL' factorization; it
 * is converted to one if it isn't. */
...
{
    return (CHOLMOD(updown_mask3) (C, D, NULL, NULL, 0, L, NULL, NULL,
	Common)) ;
}
...
/* Same as cholmod_updown but allows more entries
 * */
...
{
    return (CHOLMOD(updown_mask4) (update, C, NULL, NULL, 0, L, NULL, NULL,
	Common)) ;
}
...
/* Does the same as cholmod_updown2, except that it also updates/downdates the
 * solution to Lx=b+DeltaB.  x and b must be n-by-1 dense matrices.  b is not
 * need as input to this routine, but a sparse change to b is (DeltaB).  Only
 * entries in DeltaB corresponding to columns modified in L are accessed; the
 * rest are ignored.
...
{
    return (CHOLMOD(updown_mask3) (C, D, NULL, NULL, 0, L, X, DeltaB,
	Common)) ;
}

int CHOLMOD(updown3_solve)
(
    /* ---- input ---- */
    int update,		/* TRUE for update, FALSE for downdate */
    cholmod_sparse *C,	/* the incoming sparse update */
...
{
    return (CHOLMOD(updown_mask4) (update, C, NULL, NULL, 0, L, X, DeltaB,
	Common)) ;
}
...
/* The sparsity pattern of C and D must be the same
 * If they are not the same, the caller must
 * fill the disjoint sets with 0s
 *
 *
 * */
int CHOLMOD(updown_mask3)
(
    /* ---- input ---- */
    cholmod_sparse *C,	/* the incoming sparse update */
    cholmod_sparse *D,	/* the incoming sparse downdate */
...
)
{
	...
	*Set_ps1 [32], *Set_ps2 [32], *ps1, *ps2 ;
    size_t maxrank ;
    Path_type OrderedPath [32], Path [32] ;
    Int n, wdim, k1, k2, npaths, i, j, row, cpacked, dpacked, ccol, p, cncol, dncol, do_solve,
	mark, jj, j2, kk, nextj, p1, p2, c, use_colmark, newlnz,
	k, newpath, path_order, w_order, scattered, path, newparent, pp1, pp2,
	smax, maxrow, row1, nsets, s, p3, newlnz1, Set [32], top, len, lnz, m,
	botrow ;
    ...
    RETURN_IF_XTYPE_INVALID (L, CHOLMOD_PATTERN, CHOLMOD_REAL, FALSE) ;
    RETURN_IF_XTYPE_INVALID (C, CHOLMOD_REAL, CHOLMOD_REAL, FALSE) ;
    RETURN_IF_XTYPE_INVALID (D, CHOLMOD_REAL, CHOLMOD_REAL, FALSE) ;
    n = L->n ;
    cncol = C->ncol ;
    if (!(C->sorted))
    {
    ...
    if (n != (Int) (D->nrow))
    {
	ERROR (CHOLMOD_INVALID, "D and L dimensions do not match") ;
	return (FALSE) ;
    }
    do_solve = (X != NULL) && (DeltaB != NULL) ;
    if (do_solve)
    ...
    /* w = 2 * wdim * n */ 
    /* For this funtion it is 2*wdim because we are doing updates and downdates together*/
    w = CHOLMOD(mult_size_t) (n, 2 * wdim, &ok) ; // Roger note: mult_size_t is safe size_t mult while checking for overflow
    if (!ok)
    {
	ERROR (CHOLMOD_TOO_LARGE, "problem too large") ;
	return (FALSE) ;
    ...
    /* Allocate (nrow, iworksize, xworksize) of space */
    CHOLMOD(allocate_work) (n, n, w, Common) ;
    if (Common->status < CHOLMOD_OK || maxrank == 0)
    {
	/* out of memory, L is returned unchanged */
    ...
    Cnz = C->nz ;
    cpacked = C->packed ;
    ASSERT (IMPLIES (!cpacked, Cnz != NULL)) ;

    PRINT1 (("updown, rank %g update %d\n", (double) D->ncol, update)) ;
    DEBUG (CHOLMOD(dump_factor) (L, "input L for updown", Common)) ;
    ASSERT (CHOLMOD(dump_sparse) (D, "input D for updown", Common) >= 0) ;

    Di = D->i ;
    Dp = D->p ;
    Dnz = D->nz ;
    dpacked = D->packed ;
    ASSERT (IMPLIES (!dpacked, Dnz != NULL)) ;
    ...
    /* Do k columns of C and D at a time */
    for (k1 = 0 ; k1 < cncol ; k1 += k)
    {
	...
	for (ccol = k1 ; ccol <= k2 ; ccol++)
	{
	    ...
	    ASSERT (ccol >= 0 && ccol <= cncol) ;
	    pp1 = Cp [ccol] ;
	    pp2 = (cpacked) ? (Cp [ccol+1]) : (pp1 + Cnz [ccol]) ;
	    /* get the row index j of the first entry in C (:,ccol) */
	    if (pp2 > pp1)
	    {
		/* Column ccol of C has at least one entry. */
	    ...
	    {
		/* Column ccol of C is empty.  Pretend it has one entry in
		 * the last column with numerical value of zero. (Roger Note: is it last row* )*/
		j = n-1 ;
	    }
	    ASSERT (j >= 0 && j < n) ;
	...
	while (j < n)
	{
	    ...
	    {
		PRINT1 (("Col realloc: j "ID" newlnz "ID"\n", j, newlnz)) ;
		if (!CHOLMOD(reallocate_column2) (j, newlnz, L, Common))
		{
		    /* out of memory, L is now simplicial symbolic */
		    CHOLMOD(clear_flag) (Common) ;
		    for (j = 0 ; j <= n ; j++)
...
/* ========================================================================== */
/* === cholmod_updown_mask4 ================================================= */
/* ========================================================================== */

int CHOLMOD(updown_mask4)
(
    /* ---- input ---- */
    int update,		/* TRUE for update, FALSE for downdate */
    cholmod_sparse *C,	/* the incoming sparse update */
