//------------------------------------------------------------------------------
// CHOLMOD/Cholesky/t_cholmod_rowfac2: template for cholmod_rowfac2
//------------------------------------------------------------------------------
...
/* Template routine for cholmod_rowfac2.  Supports any numeric xtype
...
#ifdef MASK
static int TEMPLATE (cholmod_rowfac2_mask)
#else
static int TEMPLATE (cholmod_rowfac2)
#endif
(
    /* ---- input ---- */
...
)
{
	...
	*Iwork ;
	...
	/* workspace: none */
	CHOLMOD(change_factor2) (A->xtype, is_ll, FALSE, FALSE, TRUE, L, Common);
